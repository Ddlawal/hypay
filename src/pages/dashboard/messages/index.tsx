...
type messageCardProps = {
    messageDetails: { index: number; name: string; message: string; time: string; date: string }
}

export const MessagesCard = ({ messageDetails }: messageCardProps) => {
    const { name, message, time, date, index } = messageDetails
    return (
        <div>
            <Card className="my-3 hidden items-center justify-between rounded-md p-3 pl-8 text-sm shadow-xl md:flex">
                ...
                <div className="flex w-[20%] items-center justify-between">
                    <NextLink href={`/dashboard/messages/${index}`}>
                ...
                <div className="md:w-10/12">
                    ...
                    <main className="">
                        {dummyMessages.map((item, index) => (
                            <MessagesCard key={index} messageDetails={{ ...item, index }} />
                        ))}
                    </main>
                </div>
            </div>
